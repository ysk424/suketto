...
with tempfile.TemporaryDirectory(prefix='suketto-live-') as directory:
    try:
        ...
        r.send('動作確認です。blender_scene ツールで現在の Blender バージョンを確認し、日本語で一文だけ答えてください。ファイルやシーンは変更しないでください。', 'gpt-6-luna', 'medium')
        wait_for(lambda: not r.busy)
        print('LUNA_RESPONSE', r.messages, flush=True)
        ...
        r.send('次は制作の動作確認です。blender_python を使い、位置 (0,0,0)、寸法 (1,2,3)、名前「助人_接続確認」の立方体を一つ追加してください。他のオブジェクトは変更せず、ファイル保存やレンダーは不要です。結果を日本語で短く答えてください。', 'gpt-6-sol', 'medium')
        ...
        print('SOL_MODEL_CREATION_OK', obj.name, tuple(obj.dimensions), flush=True)
        print('LATEST_RESPONSE', [x['text'] for x in r.messages if x['role']=='assistant'][-1], flush=True)
        sid = r.session_id
        r.new_chat()
        r.load(sid)
        assert r.resume_needed
        r.send('先ほど作ったオブジェクトの名前だけ答えてください。ツール実行は不要です。', 'gpt-6-luna', 'medium')
        wait_for(lambda: not r.busy)
